...
public class KjyController {
	...
	public String emailAuth(@RequestParam String m_email) throws Exception {
		...
		sendMail.setText(new StringBuffer().append("<h1>이메일 인증</h1><br/>")
	...
	// 회원 탈퇴 처리 (경매 진행 중인 회원 확인)
	@RequestMapping(value = "/deleteMemberRun", method = RequestMethod.POST)
	public String deleteMemberRun(HttpSession session, String m_id, String m_pw, RedirectAttributes rttr) throws Exception {
		MemberVo memberVo = memberService.selectMember(m_id);
		String getM_pw = memberVo.getM_pw();
		if (!(getM_pw.equals(m_pw))) { // 비밀번호 확인
			rttr.addFlashAttribute("msg", "checkPw");
			return "redirect:/kjy/member/deleteMemberForm";
		} else {
			boolean result = memberService.auctioningMember(m_id); // 경매 진행 중인 회원 확인
			System.out.println("result:" + result);
			if (result == true) {
				memberService.deleteMember(m_id, m_pw); // 널인 경우만 삭제
				session.invalidate();
				return "redirect:/";
			} else {
